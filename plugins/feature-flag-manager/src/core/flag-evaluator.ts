...
export class FlagEvaluator {
  ...
  async evaluate(flagId: string, context: EvaluationContext): Promise<FlagEvaluation> {
    ...
    const override = this.store.getOverride(flagId);
    if (override) {
      return {
        flagId,
  ...
  private async evaluateDependencies(
    ...
    for (const dependency of flag.dependencies) {
      const dependentFlag = this.store.getFlag(dependency.flagId);
      if (!dependentFlag) {
  ...
  private evaluateTargeting(
    ...
    if (flag.targeting.userSegments && flag.targeting.userSegments.length > 0) {
      const userSegments = this.store.getState().userSegments;
