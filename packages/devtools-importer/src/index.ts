...
export type TanstackDevtoolsImporterOptions = {
  ...
  /**
   * Choose a fixed port OR let the plugin pick a random port in a range during dev.
   */
  port?: number;
...
export function tanstackDevtoolsImporter(
  opts: TanstackDevtoolsImporterOptions
): Plugin {
  const {
    plugins,
    config = {},
  ...
  if (!Array.isArray(plugins) || plugins.length === 0) {
    throw new Error(
      "[tanstack-devtools-importer] `plugins` must be a non-empty array of import specifiers."
    );
  }
    ...
    async config(user: UserConfig, env) {
      ...
      if (shouldEnable) {
        if (typeof port === "number") {
          ...
          // dev-only random port
          selectedPort = await findAvailablePort(portRange.min, portRange.max);
        }
    ...
    async load(id) {
      ...
      if (id === R_PLUGINS) {
        ...
        const checks = await Promise.all(
          plugins.map(async (spec) => ({ spec, ok: !!(await this.resolve(spec)) }))
        );
        const bad = checks.filter((c) => !c.ok);
        if (bad.length) {
          this.error(
            `[tanstack-devtools-importer] Could not resolve:\n` +
